...
for region in regions:
    ...
    region_data_17 = data_17[data_17['시도명'] == region]  # 17년도 지역 데이터
    region_data_19 = data_19[data_19['시도명'] == region]  # 19년도 지역 데이터

    closed_stores_in_region = region_data_17[~region_data_17[['상가업소번호']].apply(tuple, axis=1).isin(region_data_19[['상가업소번호']].apply(tuple, axis=1))]
    open_stores_in_region = region_data_19[~region_data_19[['상가업소번호']].apply(tuple, axis=1).isin(region_data_17[['상가업소번호']].apply(tuple, axis=1))]

    for category in categories:
        if category == '관광/여가/오락':
            total_stores_in_category_17 = region_data_17[
                (region_data_17['상권업종대분류명'] == category) & (region_data_17['상권업종중분류명'] != '스포츠/운동')]
            ...
            total_stores_in_category_17 = region_data_17[region_data_17['상권업종대분류명'] == category]
        ...
        overall_closure_open_rates[category]['총점포'] += len(total_stores_in_category_17)
    ...
    # '스포츠/운동' 업종 추가 계산
    total_stores_in_sports = region_data_17[
        (region_data_17['상권업종중분류명'] == '스포츠/운동') | 
        (region_data_17['상권업종대분류명'] == '스포츠') | 
        (region_data_17['상권업종중분류명'] == '학원-예능취미체육')
    ]
...
detailed_closure_open_df.to_csv('./detailed_closure_open_17_19.csv', index=False)
overall_closure_open_df.to_csv('./overall_closure_open_17_19.csv', index=False)
